...
| **`elk_logstash_host`**     | (none)            | Hostname or IP of a Logstash/ELK endpoint to which DNS logs should be shipped. Pair this with the `data_systems/logstash` role (and optionally `data_systems/kibana`) to deploy the central logging stack within this repository, or point it at an external service. This variable is typically consumed by Filebeat or other shippers to reach the Logstash Beats input. |
...
- hosts: dns_servers
  ...
  vars:
    ...
    # elk_logstash_host: "{{ groups['logstash_servers'][0] | default('logstash.intra.example.com') }}"  # (optional) set when shipping logs via data_systems/logstash
