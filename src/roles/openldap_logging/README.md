...
| -------------- | --------------------------------------- | ----------- |
| **`elk_host`** | "logstash.example.com:5044" (example) | Hostname and port of the Logstash or log collector endpoint where Filebeat should send OpenLDAP logs. By default, if `elk_host` is not set by the user, the Filebeat configuration will assume a Logstash server at **`logstash.example.com:5044`** as a placeholder. **Override this** in your inventory or playbook with the actual host:port of your ELK stack’s Logstash (or other Beats input) to ensure logs are delivered to your central server. |

**Note:** The above default is a placeholder for demonstration. In a real deployment, you must provide a valid `elk_host` value (or set up DNS such that `logstash.example.com` resolves correctly) for log forwarding to function. If your environment uses a different pipeline (for example, sending logs directly to Elasticsearch or using a cloud service), you would need to adjust the Filebeat output configuration accordingly (this role, as written, assumes a Logstash endpoint by default).
