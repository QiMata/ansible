...
**Ansible Collections:** This role uses only modules from Ansible’s built-in collection (i.e. `ansible.builtin`). You do not need any specific Ansible Galaxy collections for the role’s tasks. Modules like `user`, `file`, `command`, `pip`, `template`, and `systemd` are all part of Ansible core (or the default included modules). However, note that the wider repository (playbooks and other roles) may rely on certain collections. In particular, the project’s overall requirements include **community.general** (>= 8.4.0) and **community.postgresql** (>= 3.4.0). Ensure those collections are installed if you are running this role as part of the larger playbook that includes roles (e.g., for PostgreSQL or other components) which require them.
...
* **Metadata database (Neo4j)** – The Neo4j graph database that the metadata service uses to store catalog data. You may deploy Neo4j using a `neo4j_server` role (or `neo4j` role) in this repository, or ensure it’s provisioned separately.
* **Search index (Elasticsearch)** – An Elasticsearch cluster or service that stores the search index for Amundsen. The search service uses this as its backend for search queries. You can deploy one via the `elasticsearch_cluster` role (or related Elasticsearch roles in this repository), or use an existing Elasticsearch service.
...
* **[neo4j (neo4j_server)](../neo4j/README.md)** – *Neo4j Database role*. Sets up a Neo4j graph database server. Amundsen Metadata service uses Neo4j as its backend database to store the metadata graph (tables, users, relationships, etc.). Deploying Neo4j (via this role or otherwise) is necessary for the metadata service to function. In the context of Amundsen, Neo4j often runs on a separate host or the same host as metadata service, and it should be secured (it listens on a bolt port, default 7687, and possibly an HTTP port for browser).
* **[elasticsearch_cluster](../elasticsearch_cluster/README.md)** – *Elasticsearch Cluster role*. Provisions an Elasticsearch service or cluster, which is used by Amundsen Search as the search index backend. This role (or set of roles, as the repository might break Elasticsearch setup into multiple roles) will handle installing Elasticsearch, configuring it (possibly with clusters, multi-node setup, etc.), and opening the necessary ports. Amundsen Search relies on Elasticsearch but does not install it, so you’ll use this to ensure an ES instance is available.
