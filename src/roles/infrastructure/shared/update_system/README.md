...
*Note:* Generally, no other variables need to be set for this role to run. The above defaults are defined in **`roles/update_system/defaults/main.yml`** and can be overridden if necessary (e.g., in an inventory or playbook). Typically you would only change these if you have specific policies about the types of upgrades or post-upgrade cleanup.
